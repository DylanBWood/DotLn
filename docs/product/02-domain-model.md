...
| **Reactor**        | A pure function `(state, event, env) → Decision`. No hidden clock, RNG, I/O, or model call. **`env` is a pure projection of the log**: clock ticks, presence changes (`OperatorPresenceChanged` away/returned), and resource-pressure changes enter the system as events; post-draw RNG state is carried in kernel state; replay never consults anything outside the log. Reactor purity is an author obligation the kernel does not enforce at runtime: the kernel's own functions consult no ambient source (pinned by poisoning tests), but a reactor that reaches for `Date.now()` silently breaks replay identity. `replay(initial, events, reactor, predicates, projectEnvironment?)` accepts a pure per-step projector `(state, event) => Omit<KernelEnv, "now" \| "predicates">`. It receives current pre-step state and returns a finite RNG seed and optional policy; a supplied non-finite seed refuses before the reactor. Replay owns event time and the predicate registry. Projector purity is also an author obligation. With no projector, exported `defaultEnvironmentProjection` preserves the reserved-key fallback: numeric `state.rngState`, otherwise `0`, and `state.policy` promoted when present (including null). The legacy numeric check is unchanged; explicit projections let each application own its state layout.                        |
...
| **PresencePolicy** (compiled subset)    | Versioned instance doctrine mapping recorded presence, clock, threshold, outcome, return, and reset events into independent changes to attention priority, work-scope budget, and effect authority; external capability is observed, never produced. A transition declares its source grant, event predicate, stage, active window, ceiling, stop/reset, and replenishment/loop behavior. A policy may hold, shrink, grow, peak, step, stop, reset, or loop any supported output; absent a declared transition, that output holds. Selecting a broader `AuthorityEnvelope` is actual authority growth, while enlarging task/time/token/work volume alone is not. WO-067 implements the bounded progressive phase contract below; broader curves and profile selection remain candidates under ADR-0007.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
#### Compiled presence policies

WO-067 adds optional `presence`, omitted when empty in both normalized graph
and program. Policies sort by `policyId`; phase order is executable data and
is preserved. Code DSL carries the collection, the function table uses
`presence-policy` rows, and statechart JSON uses `context.presence`.

`PresencePolicy` carries `policyId`, positive integer `version`, all four
`axes` (`attention`, `work-scope`, `effect-authority`, `external-capability`),
ordered nonempty `phases`, `curve: "progressive"`,
`returnRule: "cancel-on-return"`, and `decay: { idleMs, expires: "phase" }`.
Unknown fields or rules, duplicate/reserved phase ids (`present`, `expired`),
invalid numbers and unsupported cadence kinds refuse with a field path.

Each phase declares `phaseId`, `entry` (exactly one `cadence` or predicate
`condition`), `attentionPriority`, `scope: { surfaces, changeSize: { files,
lines }, budget }`, `envelope`, `requiredCapabilities`, `discretionary` and
`inFlightOnReturn: "finish" | "kill"`. Scope surfaces are named work surfaces;
the host resolves them and counts actual files/lines. Budget allocation and
attention do not grant effects. Non-discretionary work must use `finish`.

`envelope` is a restriction relative to the final compiled base: optional
`allowedEffects` is a subset, `resourceLimits` cannot raise an existing limit
or create a positive allowance, and `expiresAt` cannot extend expiry.
`deniedEffects`, `requiredEvidence`, `revocationEventTypes` and
`revocationConditions` add to the base; empty lists cannot erase it. Terminal
prefix allowances retain inherited denials. A literal base-denied allowance
refuses with `AUTHORITY WIDENING`. Growth between phases can restore a base
allowance but cannot remove a support restriction or admit a graph-only grant.

Compiled policies carry the base envelope id and admitted source grant ids;
each phase carries its effective envelope, gated kernel-compatible cadence,
cancel predicates, statechart gate and observed capability availability. Entry
accepts only `Once`, `After`, `Every`, `Gate` and `Until`; condition-only entry
lowers to `Gate(After(0), condition)`. Unknown predicates remain a host registry
requirement, not an implicit true condition. Missing external capabilities emit
`NoOp` with a reason. Compile-time availability is a snapshot; the host must
recheck capability before dispatch. The compiler never provisions an adapter.
...
The component tuples match the participating manifest exactly. Each digest identifies the matching normalized source definition, including otherwise un-emitted metadata; group and mechanism projections remain manifest fields. The exact public preimage and domain are pinned in [04](04-interfaces.md#editable-view-v1-normalization-and-semantic-hash). `compilationEnvironment` retains `environmentId`, numeric `version`, capability list, `repo`, and `baseCommit`, plus the optional host-owned `authorityGrantRegistry` when the host supplied one; the skeleton admits that field only when it normalizes as a grant list, so a grant-bearing identity equips and reaches the source-change host while a malformed registry still refuses (WO-064 D006, WO-157). Expiry is also retained explicitly and in the raw graph. These are equality receipts, not unique, cryptographic, or authenticated identities.
...
| Term                    | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Event store**         | Append-only source of truth. The JSONL codec accepts only the empty string or newline-terminated EventEnvelope schema-1 lines with required string fields, finite numeric occurredAt, optional string IDs, arbitrary JSON payload values, sequential `evt_<n>` identity and no unknown envelope keys. `tryDecodeLog` returns a typed code/path/message failure naming the physical line; decode and append throw that diagnostic. Append retains `evt_<validated line count + 1>` assignment. Payload meaning stays with reactors; historical logs are not rewritten (WO-045). Persistence progression: JSONL → SQLite (when transactionality/outbox matter). Markdown is a _generated projection_, never shared mutable truth.                                                                                     |
