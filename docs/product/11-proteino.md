...
while an operator observes them, addresses a chosen resident or group in prose,
...
Operator analogies communicate **product shapes**. Their load-bearing content is
usually a relationship, interaction, transition, contrast, feedback loop, or
felt quality—not a claim that every detail of the source is historically or
technically exact. Synthesis therefore starts from the desired experience and
works down the abstraction ladder:

1. **felt shape** — what should become possible, legible, surprising, or
   satisfying;
...
A loose analogy can carry a precise product shape. Do not reject or flatten the
shape because the analogy is imperfect, and do not copy accidental source
details into the ontology. When a literal detail becomes necessary to the
requirement, verify it or leave it explicitly unresolved. **The shape chooses
the experience; evidence chooses the mechanism.** This is a standing
interpretation rule for the operator's earlier and future ideation, not just for
this application.
...
1. **Observe.** Enter a small, persistent social world and notice individuals,
   groups, activities, relationships, problems, and opportunities already in
   motion.
2. **Select.** Choose one resident, a deliberately assembled set, a team, or a
   larger audience. Selection expresses intended recipients; it does not imply
...
This is not a chat window placed beside a simulation. Language is one of the
world's action systems, and its reception, non-reception, mutation, and
downstream consequences are part of the playable state.
...
inspection trail should be capable of distinguishing:

```text
operator source
  → proposed system interpretation
  → delivered expression and recipients
  → reported or derived recipient interpretation
  → temporary or reusable mechanism that participated
  → decision / intent
  → world event
  → evaluated outcome
```

Every arrow can be absent, partial, delayed, or uncertain. That is useful
evidence. A visible intended-versus-derived interpretation mismatch can support
the label _misunderstood_; generated narration alone cannot establish access to
a resident's inner thought. The application shows the evidence and derivation
...
compare the temporary behavior with the candidate reusable definition, see what
...
The executable-pattern thesis becomes intuitive when its mechanics can be tried
on a living world. A book, method, coaching style, or private doctrine can
define one or more observable contrasts between a starting tendency and a target
tendency. Residents, groups, relationships, activities, and event patterns can
be subjects on those dimensions. Residents or groups may receive different
patterns, doses, delivery styles, and timings; the operator then compares the
chosen subjects' decisions, structure, performance, propagation, and side
effects. The message recipient and the subject of comparison are not assumed to
be the same entity.

The contrast is not automatically one `bookStart → bookEnd` scalar. A source may
contain several independent or conflicting dimensions, each requiring an
operational definition and evaluator. A personal style remains labeled as
personal rather than masquerading as sourced public doctrine. Simulation
evidence establishes behavior inside a declared world/model configuration; it
does not establish that the source works on real people.
...
- **participation:** an inspectable decision or mechanism cites the
  intervention;
- **paired simulation evidence:** branches share a named checkpoint, world and
  component versions, controlled randomness policy, and declared differences;
- **first divergence:** the earliest observable point at which the branches
  separate;
...
observations such as attempts, scores, travel, time, or injuries remain distinct
...
usable, it either supports meaningful interaction or is visibly unavailable;
...
These references retain the operator's intended shapes without making their
literal details architectural dependencies:
...
| A basketball squad                                                        | Interventions can range from individual technique through team structure to ordinary wellbeing, with different outcomes and timescales.                                                                        |
| The operator's _Grand Theft Auto_ experience                              | The world honors apparent activities with deep, activity-specific interaction while retaining open exploration.                                                                                                |
| The operator's recollection of varied game reactions                      | Combinations at scale can reward curiosity with rare behavior, then reveal reproducible lineage after discovery. No claim about engine history or a required animation technology follows.                     |

## Candidate first evidence slice

A small basketball squad is a promising first vertical because one setting
contains several scales without pretending to model everything: individual
shooting mechanics, a team offense, advice about blister prevention, game
results, communication events, and declared teamwork proxies. A baseline
checkpoint and a paired intervention branch could demonstrate selection,
long-form input, interpretation inspection, delayed effects, direct and latent
measures, first divergence, and replay in one coherent world.

That scenario is a candidate, not a roadmap insertion. The first proof should be
chosen for its ability to establish the loop with honest evidence and a
satisfying feel. It should not begin by building a universal town, a complete
sports simulator, or a library of every source pattern.
...
Until those choices are separately evidenced and authorized, προτείνω selects a
product direction, not a stack, schema, simulator, model, or delivery date.
