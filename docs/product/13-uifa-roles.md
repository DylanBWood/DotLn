...
| Devops       | `resume`, `worktree`, `release`, `work-orders`, publication/evidence gates; disposable inspection workers, leases, `dotln status`, and WO-022 metadata sense profiles                                                                               | A relocatable control plane and exported launchpad (WO-033); the evidence runner (WO-036); the documentation structure reset (WO-035); general worker profiles and recurring-procedure automation beyond the bounded inspection host |
