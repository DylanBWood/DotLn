...
The Contributor supplies the first harness target: a saved build lowers to
observed project permissions/hooks, role skills and explicit residue. The
engineer can inspect each file's origin and hash, test unit removal and regenerate
without creating a second hand-written behavior system. [WO-039's receipt](../evidence/WO-039/README.md)
limits that claim to the observed profiles and declared host facts.
...
The plan refuter now supplies one bounded tester surface: an independent
Contra-Auguste episode judges proposed orders against named vision passages,
capability rows and human roles, then emits an immutable receipt. Its planning
gate checks dispositions and staleness; it does not verify implementations or
replace the person's judgment about the proposed shape. The
[receipt convention](../planning/refutations/README.md) records that boundary.
...
`npm run harness -- emit` and `harness check` now make this repository's
Contributor configuration reproducible and detect drift. Role dispatch replaces
the mandatory whole-guide startup read; the matched fixture measurement and
live read observer expose its limits. Missing Codex hooks remain unavailable,
and personal sandbox/settings controls remain outside the project emitter.
...
names what exists today and which planned rung serves the role next. The
WO-032 actor board is implemented for `v0.14.0`; its
[question-to-cell fixture](../../packages/console/fixtures/role-answers.json)
pins a concrete answer for each role. Independent verification reviews that
mapping against the questions above. A role label alone is not service evidence.

| Role         | Today                                                                                                                                                                                                                                                                                                                                                                                                                                          | Next planned rungs                                                                                                                                                                                                                                                                                                                   |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Product lead | The intake → clean-room synthesis → ledger → product-doc pipeline; the append-only idea ledger; the publication editions; WO-032's Blueprint panel links recorded capability and publication rows to the latest refutation verdict and hold state                                                                                                                                                                                              | The workstream application journeys (12); `προτείνω`'s prose-as-action authoring (11)                                                                                                                                                                                                                                                |
| Showrunner   | The generated work-order index, the planning map and its refutation gate (WO-041), control status with elapsed phases, release notes in annotated tags and GitHub Releases, per-order control segments (WO-030), control-plane Beacons (WO-021), the actor usage projection (WO-031); WO-032's Actors and Work panels join recorded actors, phases, blockers, legal actions, worktrees, releases, and evidence                                 | Lane sync and the first measured paired waves (WO-033); workstreams across repositories from a fork running the compiled build (WO-034)                                                                                                                                                                                              |
| Engineer     | LoadoutGraph v1, three equivalent editable views, the compiled diff and tooltip with declared costs, the compiled Entropy Reducer loadout; the Contributor harness target with origin-addressed hooks, role skills and residue (WO-039); WO-032's Builds and Mechanisms panels expose compiled item tooltips, three equal view hashes, declared costs, and unit evidence                                                                       | The migration batches that compile the remaining feedback shapes into that build (WO-040); the 5S set as a multi-active link group with set bonuses (WO-037); the community build workshop and the candidate multi-dimensional authorship assistance (04)                                                                            |
| Tester       | Deterministic replay and fixtures, verifier episodes, the corpus track, the walking skeleton's scenario, and blinded plan-refutation receipts with a checked planning gate (WO-041); WO-032's Actors and Mechanisms panels link verification matrices and receipts with fixture/live activation counts                                                                                                                                         | Each migration batch's regression fixtures and the whole-set measurement (WO-040); independent verification over real workers (WO-010 shipped); opinion cohorts; `προτείνω`'s paired counterfactuals                                                                                                                                 |
| Devops       | `resume`, `worktree`, `release`, `work-orders`, publication/evidence gates; disposable inspection workers, leases, `dotln status`, and WO-022 metadata sense profiles; generated project Contributor configuration with drift and context gates (WO-039); WO-032's Actors and Work panels expose transport selections, unknown effective values, leases and heartbeats, authority summaries, source availability, and separate Beacon metadata | A starter exported as a compiled build with a pinned runtime, kit and instance and overlay separation, and worktree-local harness emit for targets (WO-033); the evidence runner (WO-036); the documentation structure reset (WO-035); general worker profiles and recurring-procedure automation beyond the bounded inspection host |

### Actor board role service

WO-032 supplies five pinned answers through one `uifa-board-v1` contract.
Product lead selects the latest recorded verdict and hold state; showrunner
selects order phase, blocker evidence, legal actions and release membership;
engineer selects compiler tooltips, declared costs and view hashes; tester
selects causal fixture counts and acceptance evaluations; devops selects each
mechanism's boundary, rung and next maturity condition beside capability gaps.
The [fixture mapping](../../packages/console/fixtures/role-answers.json) names
the exact rows, cells and source records. These are implemented projection
answers; usefulness and independent acceptance retain their own evidence gates.
