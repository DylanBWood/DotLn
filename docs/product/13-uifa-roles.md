...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Product lead | The intake → clean-room synthesis → ledger → product-doc pipeline; the append-only idea ledger; the publication editions                                                                                                                                                             | The actor board's Blueprint panel (WO-032); the workstream application journeys (12); `προτείνω`'s prose-as-action authoring (11)                                                                                                                                                                                                                                                                                                                                           |
| Showrunner   | The generated work-order index, the planning map and its refutation gate (WO-041), control status with elapsed phases, release notes in annotated tags and GitHub Releases, per-order control segments (WO-030), control-plane Beacons (WO-021), the actor usage projection (WO-031) | The actor board's Work panel, the first draft's showrunner board in full (WO-032); lane sync and the first measured paired waves (WO-033); workstreams across repositories from a fork running the compiled build (WO-034)                                                                                                                                                                                                                                                  |
| Engineer     | LoadoutGraph v1, three equivalent editable views, the compiled diff and tooltip with declared costs, the compiled Entropy Reducer loadout                                                                                                                                            | The Contributor build lowered into the harness the sessions run under, settings permissions and hooks, role skills, and a marked residue block (WO-039); the migration batches that compile the remaining feedback shapes into that build (WO-040); the actor board's Builds and Mechanisms panels (WO-032); the 5S set as a multi-active link group with set bonuses (WO-037); the community build workshop and the candidate multi-dimensional authorship assistance (04) |
| Tester       | Deterministic replay and fixtures, verifier episodes, the corpus track, the walking skeleton's scenario, and blinded plan-refutation receipts with a checked planning gate (WO-041)                                                                                                  | Each migration batch's regression fixtures and the whole-set measurement (WO-040); the actor board's Mechanisms panel with observed activations (WO-032); independent verification over real workers (WO-010 shipped); opinion cohorts; `προτείνω`'s paired counterfactuals                                                                                                                                                                                                 |
| Devops       | `resume`, `worktree`, `release`, `work-orders`, publication/evidence gates; disposable inspection workers, leases, `dotln status`, and WO-022 metadata sense profiles                                                                                                                | A starter exported as a compiled build with a pinned runtime, kit and instance and overlay separation, and worktree-local harness emit for targets (WO-033); this repository running on its own generated harness configuration with `harness check` in the evidence gate (WO-039); the evidence runner (WO-036); the documentation structure reset (WO-035); general worker profiles and recurring-procedure automation beyond the bounded inspection host                 |
