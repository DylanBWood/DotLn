...
| Engineer     | LoadoutGraph v1, three equivalent editable views, the compiled diff and tooltip with declared costs, the compiled Entropy Reducer loadout; WO-032's Builds and Mechanisms panels expose compiled item tooltips, three equal view hashes, declared costs, and unit evidence                                                                                                                                     | The Contributor build lowered into the harness the sessions run under, settings permissions and hooks, role skills, and a marked residue block (WO-039); the migration batches that compile the remaining feedback shapes into that build (WO-040); the 5S set as a multi-active link group with set bonuses (WO-037); the community build workshop and the candidate multi-dimensional authorship assistance (04)                                          |
...
| Devops       | `resume`, `worktree`, `release`, `work-orders`, publication/evidence gates; disposable inspection workers, leases, `dotln status`, and WO-022 metadata sense profiles; WO-032's Actors and Work panels expose transport selections, unknown effective values, leases and heartbeats, authority summaries, source availability, and separate Beacon metadata                                                    | A starter exported as a compiled build with a pinned runtime, kit and instance and overlay separation, and worktree-local harness emit for targets (WO-033); this repository running on its own generated harness configuration with `harness check` in the evidence gate (WO-039); the evidence runner (WO-036); the documentation structure reset (WO-035); general worker profiles and recurring-procedure automation beyond the bounded inspection host |
