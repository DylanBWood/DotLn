# WO-009 — Real disposable worker, v0.4.0
...
**Cites (read these sections):** 06-roadmap.md v0.4.0; 03-architecture.md
...
`CodexCliExecWorkOrderTransport` — with the canonical launch shape (fresh
bounded invocation, project+local settings only, ambient/auto memory disabled
— external memory is deliberate — isolated worktree, explicit model + effort,
no session persistence, JSON schema output), a deterministic worktree
lifecycle, `dotln status`, and failure-injection rows 2, 4, 6.
...
- Verifier episodes are v0.5.0's; none here.
...
6. Launch-shape assertions: setting sources restricted to project+local;
   ambient auto-memory disabled by the mechanism WO-004 observed; no session
   persistence; explicit model and effort recorded in the episode's events.
...
**Non-goals:** verification episodes (v0.5.0); feedback units (v0.6.0); web
