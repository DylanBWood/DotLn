...
- Application target: unassigned.
- Dependencies: typed; dependency-ready.
- References: WO-067: hard (met) — the policy whose cadences and phases the host evaluates; WO-050: hard (met) — the reserved slice the host's state lives in (the recorded split condition); WO-044: hard (met) — the launcher is designed from the unattended rows, never assumed; WO-009: satisfied-by-release (met) v0.10.0 — the transports and worker store it dispatches through; WO-027: reference-only (non-blocking) — the local-inference probe, as the fourth actor kind's reference.
- Verification: none recorded.
- Final review: none recorded.
- Release: none recorded.
