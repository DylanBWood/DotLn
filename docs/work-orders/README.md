...
[WO-147 — Resident lock contention: a host lock that vanishes between a contender's check and its read is treated as absent or contended and retried, never thrown, so a presence command or a resident transaction under live contention waits instead of failing (version assigned at activation)](WO-147-resident-lock-contention.md)
