...
- State: draft.
- Application target: unassigned.
- Dependencies: typed; blocked on WO-075, WO-076, WO-124, WO-112, WO-111, WO-066, WO-117, WO-123.
- References: WO-075: hard (unmet) — a starter instance with the build; WO-076: hard (unmet) — the instance's overlay; WO-121: hard (met) — presence with origin; WO-122: hard (met) — the cli-worker and human-handoff actors; WO-100: hard (met) — derivation inside a portfolio; WO-120: hard (met) — derived work as durable records; WO-124: hard (unmet) — surfaces derived from the contract; WO-112: hard (unmet) — the loop proven from core first; WO-111: hard (unmet) — the unattended hour; WO-066: hard (unmet) — the pull-request loop with dispositions; WO-114: hard (met) — the run is visible in the status projection; WO-117: hard (unmet) — the run is visible in the live console; WO-123: hard (unmet) — the resident admits a filed intent under standing authorization and owns the vertical continuation.
- Verification: none recorded.
- Final review: none recorded.
- Release: none recorded.
