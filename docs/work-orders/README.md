...
- State: draft.
- Application target: unassigned.
- Dependencies: typed; blocked on WO-051.
- References: WO-068: hard (met) — the catalog; WO-051: hard (unmet) — the writer request and profile the cli-worker kind dispatches; WO-044: hard (met) — the detached-launch rows the launch path is designed from.
- Verification: none recorded.
- Final review: none recorded.
- Release: none recorded.
