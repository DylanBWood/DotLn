...
- State: draft.
- Application target: unassigned.
- Dependencies: typed; blocked on WO-123, WO-053, WO-056, WO-047.
- References: WO-123: hard (unmet) — the composition this run executes; WO-053: hard (unmet) — the live source-change primitive; WO-056: hard (unmet) — independent verification and repair, live; WO-045: hard (met) — the codecs are mandatory before the loop runs against anything the operator keeps; WO-046: hard (met) — a persisted continuation of an unsupported kind must fail at decode; WO-047: hard (unmet) — the second state shape replays through an explicit projector; WO-048: hard (met) — host recovery refuses malformed state before dispatch.
- Verification: none recorded.
- Final review: none recorded.
- Release: none recorded.
