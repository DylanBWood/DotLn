...
Preflight per order (while you still have network). Before activation, assign the version/close disposition and confirm the three-role `**Effort:**` declaration beside the `**Model:**` declaration still matches the planner's intent. The 2026-09-04 planning pass declared `executor xhigh+; verifier xhigh+; reviewer any` on every remaining draft so activation does not stall on the WO-019 check; the control plane still has no global effort default, and a lower floor for a mechanical order is granted only by a dated amendment to that order's line. In the new worktree: provision node_modules (`npm ci`, or copy from the main checkout — the recorded Codex sandbox is network-disabled workspace-write, so nothing can be installed mid-flight); run `npm run build && npm test` and confirm green at the base commit; record the base commit SHA — several deliverables are keyed to it. For WO-107 additionally schedule a quiet machine window (close other heavy processes); WO-108 wants a mostly-quiet machine too but tolerates noise. Then hand off: cd, launch codex (it reads AGENTS.md → CLAUDE.md), give `resume: next` (in-slot) or the work-order path (out-of-slot). Every order is designed for zero mid-task judgment — if Codex asks a question, the answer is in the order or the run should stop and you triage on return.
