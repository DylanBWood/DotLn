...
**Series note:** reserved adjacent WO-10x series, parallel to the mainline;
...
structural authorization guard, the own-property resource rule, the Command
outbox protocol, CommandPersisted/CommandResult/CommandRefused);
...
`applyCommandResult`, `replayOutbox` in `packages/kernel/src/core.ts` — the
read-only oracle, including the ordered refusal chain, the trailing-`*`
prefix `effectMatches`, and the `Object.hasOwn` resource rule;
...
{effect string vs non-string} × {now before/at/after expiresAt — expiry is
strict `now > expiresAt`, so the at-boundary cell is meaningful} ×
{revocation event types 0/1/many, matching/non-matching revokedBy} ×
{denied/allowed pattern shapes: exact, bare `*`, prefix `a*`, overlapping
deny-vs-allow} × {required evidence subset/superset/empty} × {resource
undefined / present / limit 1/0/negative / prototype-key names `__proto__`,
`constructor`, `toString` exercising the Object.hasOwn rule}. The oracle for
reason strings is the SHIPPED code: pin the exact shipped strings (e.g.
"resource limit exceeded", "required evidence missing"), never paraphrases.
An independent in-harness precedence oracle computes each cell's expected
winner from the documented seven-step order and is compared cell-by-cell to
shipped output. Expected-output schema per cell: REFUSED cells pin the reason
and the trace branchPath `['refused', reason]`; AUTHORIZED cells return no
trace — they pin the minted commandId and the returned envelope's decremented
resourceLimits instead. The never-throws property holds over every cell.
...
malformed events through `replayOutbox` in the enumerated orders, asserting
idempotence under duplicate delivery, exact branchPath classification
(`accepted`/`dedup`/`unknown`/`ignored-event-type`), and persist-once
semantics — widening shipped failure-matrix rows 1 and 3 into exhaustive
form.
...
1. `corpus/harness/generate-authority-corpus.mjs` — seeded factorial generator
   with `--write`/`--check` (regenerate-and-diff) modes; includes the
   independent precedence oracle.
...
3. `corpus/harness/wo103-*.test.mjs` — cell replay, the never-throws property
   over the full factorial, threading and permutation sweeps.
...
2. Shipped output matches the independent precedence oracle for every cell —
   exact shipped reason strings, branchPaths, and for authorized cells the
   minted commandId and decremented resourceLimits — or the divergence is a
   quarantined numbered finding.
3. The never-throws property holds over the full factorial (refusals are
   structural, with trace).
4. Outbox sweeps prove duplicate-delivery idempotence and exact branchPath
