...
- **ZAMM method-notebook agents** `preserved` `recovered`
  - Reacting to the ZAMM scientific-method passage: 'if an agent had 1-6 that would be insanity' — plausibly ZAMM's six-part lab notebook (problem statement, hypotheses, experiments, predicted results, observed results, conclusions) maintained by an agent as its working discipline. Interpretation of '1-6' is inferred; the raw reaction is preserved verbatim in the source.
