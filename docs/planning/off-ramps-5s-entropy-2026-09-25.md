...
| Sort          | local residue: `.runtime` 158 MB, `docs/control/local` 136 MB; `harness prune` preview 207 candidates, 146.9 MB  | the operator ran `--apply`: 36 min, then refused when a gate run changed a snapshot; the planner's re-run was killed at 54 min; neither removed a byte, because the publication check makes two network calls per release row per lane (up to 728); the rewrite is WO-160 item 7 at the operator's direction (map candidate 11) |
