...
classification boundary. This revision repairs that criterion: origin is a
pure function of the hook event kind and the resident's stamp, tool
activity from any session is actor liveness, a person's submitted prompt
or an explicit `back` is the only return signal, and WO-044 gains the row
that says which hook events a scripted prompt fires. All three accepted
dispositions are in
[critical-path-2026-09-08-dispositions.json](critical-path-2026-09-08-dispositions.json)
for the next receipt, which the operator requests the same way; until it
is filed, the plan gate refuses this heading by design, and a third
consecutive hold would stop this pass by rule. No hold is overridden.
...
| Refutation gate cost                                        | The subject holds seventy-six orders; neither transport had budget, so both receipts are direct-session reviews with session-attested independence.     | The command, the direct-session form and the override route are recorded above; WO-125 makes the Codex transport selectable at `max`; two holds were answered by changed criteria, and the next receipt is the last before the three-hold stop. | A third consecutive hold stops the pass by rule.                                                                             |
