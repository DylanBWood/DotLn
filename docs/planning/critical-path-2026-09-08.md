...
validated rules applied by hand (§Quality gates applied). Neither transport
had budget for the seventy-six-order episode, so the operator requested the
judgment directly in a Codex session under the receipt convention's
direct-session form. Receipt
[2026-09-08-critical-path-002](refutations/2026-09-08-critical-path-002.md)
judged the subject at `bdbe13e`: fifty-four orders advance a thesis,
twenty-two are machinery, none drifts, and the verdict is a hold on two
criteria. WO-123 criterion 1 let a command-only fixture pass while WO-118's
one-intent resident run had no admission or scheduling path; WO-055
criterion 1 took surfaces and commands from a read-only verifier's
references, so a finding could widen a writer's scope. Both criteria were
repaired at `7e2c474`, WO-118 names WO-123 as a hard input and WO-100
names the `intent` class as WO-123's. The second receipt
[2026-09-08-critical-path-003](refutations/2026-09-08-critical-path-003.md),
from the same review source, accepted both repairs (fifty-two orders
advance a thesis, twenty-three are machinery) and held once more, on WO-121
criterion 1, labeling that order drift: its objective counted a heartbeat
from any session the resident did not launch as human origin, so an
operator-launched worker that kept working after `away` could end the away
phase, and criterion 1 tested preassigned labels rather than the
classification boundary. The repair at `b812128` made origin a pure
function of the hook event kind and the resident's stamp, tool activity
from any session actor liveness, a person's submitted prompt or an explicit
`back` the only return signal, and gave WO-044 the row that says which hook
events a scripted prompt fires. The third receipt
[2026-09-08-critical-path-004](refutations/2026-09-08-critical-path-004.md),
from the same review source, passes: forty-eight orders advance a thesis,
twenty-eight are machinery, none drifts, every hold is answered by a dated
accepted disposition in
[critical-path-2026-09-08-dispositions.json](critical-path-2026-09-08-dispositions.json),
and the plan gate admits this heading. Its largest remaining gap, the
authoring journey through the live client at WO-117 criterion 2, warrants
no hold for this horizon and is preserved as a map candidate; the receipt
also relabels five orders (WO-043, WO-062, WO-065, WO-077, WO-079) as
machinery that the two earlier receipts had called thesis-advancing, with
no criterion of theirs changed, which records judgment variance between
sessions rather than a change in the plan. No hold was overridden.
...
| E        | WO-054 → WO-055 → WO-056                                                                                                                                                                | WO-054: WO-052; WO-055: WO-054, WO-052, WO-042                                                                                                                                                                           | Verification and repair lifted onto a real worktree, then live.                                                                                                              |
...
| V        | WO-123 composition → WO-112 the loop from core → WO-118 the resident-owned loop from a starter instance                                                                                 | WO-123: the primitives, WO-068, WO-100, WO-120, WO-042; WO-112: WO-123, WO-053, WO-056, WO-045 to WO-048; WO-118: WO-112, WO-111, WO-100, WO-120, WO-121, WO-122, WO-124, WO-075, WO-076, WO-114, WO-117, WO-066, WO-123 | The resident's admission of an intent and the composition, the parity proof against a scratch target, then the product exit through the resident from a starter; R3 follows. |
...
2. **R2, mandatory, after WO-053's and WO-111's receipts.** Decide from the
   ...
   **R2 disposition (2026-09-24, corrected after VER-001):** [The first window](../evidence/WO-111/receipt-v2.json) completed three independently verified scratch orders; a separate mission store judged the DotLn worktree and recorded accurate surface drift. [The return contrast](../evidence/WO-111/return-receipt-v2.json) completed another three orders. Replay with/without its return cancels an eligible fourth dispatch; the live caller stopped before due and no in-flight return was exercised. Both windows added user Codex trust entries outside the portfolio. The receipts preserve the criterion-2 failure for re-verification. FUP-3c34a8ffbf61376f tracks runtime isolation independently of WO-111’s outcome; its completion is not a prerequisite for the current repair handoff (D014). This disposition answers planning questions without declaring the live proof passed.

   **Cost/session comparison:** the dated audit/plan baseline measured implementation spans of four to twelve hours per order, not an equivalent synthetic portfolio benchmark. WO-111's initial implementation control span is 4,566,098 ms (76.10 minutes), excluding the later verification and repair; its 648,313/536,565 ms live windows and 182,200/183,249 ms folded portfolio work are different measures. The two windows used six writer and six fresh verifier episodes plus one mission-check model episode, two operator away/back pairs and script discovery. WO-053 separately recorded two clean writer smokes, one killed-host recovery and retained failed attempts. The audit has no comparable worker-session, actor-token or dollar baseline, and WO-111 actors reported no token usage, so no cost/session reduction can be inferred. The shorter synthetic task cannot establish that the four-hour sizing problem is solved.

   **Integration-package growth trigger:** the audit inventory was 46 top-level skeleton source files (39 TypeScript, 7 JavaScript modules). At R2 the same enumeration finds 93 (80 TypeScript, 13 JavaScript modules). WO-111 adds no runtime source. The proof composes existing discovery, portfolio, source-change and verifier hosts; it also exposes a shared launch-isolation defect and an unproven combined mission/portfolio supervision path. Keep the existing slices, route the concrete isolation defect through D011's follow-up, and do not select a physical package split from count alone. Reopen package placement when a concrete host seam causes coupled changes, ownership ambiguity or measured test/build cost; a future combined supervision proof must demonstrate that composition explicitly.

   **Remaining questions:** an owner-repository portfolio still requires reviewed target selection and grants, and the containment defect must be addressed before expanding these live proofs. Starter export need not wait for WO-100, already merged; it carries the runtime at export and can update. No remaining deferred family is waived; previously closed members stay closed. The full rising-and-descending absence curve remains the [future comparative experiment](full-absence-curve-experiment-2026-09-24.md); these traces do not measure net value or ship an automatic allocator.
...
| Presence misread from actor activity                        | The first resident draft's heartbeat fired on every governed tool call; the revised WO-121 made the launch path the evidence of presence (receipt 003).                                 | WO-121 classifies by hook event kind and the resident's stamp, never by launch path; tool activity from any session is liveness; only a person's submitted prompt or an explicit `back` ends away; WO-044 records the scripted-prompt rows. | A resident that ends `away` on a worker's tool call, whichever process launched it, which WO-121's negative fixture forbids. |
...
| Integration-package growth                                  | 46 skeleton files; one decider; three new hosts on this path.                                                                                                                           | WO-050 splits state into slices before any new host branch; the audit's package split is not adopted before a seam needs it.                                                                                                                | R2 answered 2026-09-24 above: 93 top-level files; keep slices, track the demonstrated isolation seam; reopen on coupling/ownership or measured cost.                                                                                                                          |
...
| Refutation gate cost                                        | The subject holds seventy-six orders; neither transport had budget, so all three receipts are direct-session reviews with session-attested independence.                                | The command, the direct-session form and the override route are recorded above; WO-125 makes the Codex transport selectable at `max`; two holds were answered by changed criteria and the third receipt passes.                             | A third consecutive hold stops a pass by rule.                                                                               |
| Authoring journey unproven                                  | Receipt 004: no order requires a person to alter a pattern's mechanics through the live client and demonstrate the behavioral change; WO-091 to WO-095 prove compilation and rendering. | The map candidate; WO-117 criterion 2 is the place a later pass requires or scopes it; compiler results are not reported as that proof.                                                                                                     | After WO-095 and WO-117 land, or any claim of authoring usability.                                                           |
...
| The do-not-relitigate list                                                               | note §6; ADRs; `docs/lineage/resolutions.md`            | honored; the browser dependency is a note under ADR-0002's own amendment path                        | decisions                     |
...
- **Overriding the first receipt's two holds.** NoOp. Both findings held
  against the orders; an override records an operator instruction, not a
  repair. Reverse never for these two; the route stays for a hold the
  operator judges wrong.
- **Filing the resident's intent admission as its own order.** NoOp. The
  hold names WO-123 criterion 1 as the repair target, and the admission
  composes existing contracts (the draft, the portfolio, the grants, the
  continuation); WO-123 stays under the eight-item rule. Reverse if its
  implementation span exceeds four hours at R1's measure.
- **Moving the `intent` class into WO-100.** NoOp. WO-100's first
  portfolio is 5S maintenance with no remote grant; the class that needs
  remote grants belongs with the continuation that uses them. Reverse if
  WO-100's contract cannot be extended by a later order.
- **Overriding the second receipt's WO-121 hold.** NoOp. The finding held
  against the order: launch provenance is not evidence that a person
  returned. Reverse never for this hold.
- **Requiring the authoring journey through the live client in this
  horizon.** NoOp. Receipt 004 names it the largest remaining gap and says
  it warrants no hold: the equivalent-view, multi-active, set and full-5S
  orders advance the interface thesis and the runtime loop has its live
  gates. Recorded as a map candidate; reverse when WO-095 and WO-117 land or
  when any claim of authoring usability is made.
- **Inferring interactivity from process tables or input devices.** NoOp.
  Presence is never inferred; the hook event kind and the resident's stamp
  are the classifier's only inputs, and where a harness cannot tell a
  scripted prompt from a typed one the explicit commands are the only human
  signals. Reverse if WO-044's rows show that no harness fires an
  interaction event at all.
...
The repair revision's checks ran on the working tree before its commit
(subject hash
`sha256:51eb0cfccc522876f972489902dc80bd2c048802bc206349c4e4a9c3558e0988`):

- `npm run work-orders -- index` regenerated and `index --check` passed;
  the workspace subject built with 76 orders and 394 acceptance criteria and
  no parse failure; both held criteria differ from the judged subject, and
  the dispositions file names each with its date.
- The graph gained six `hard` edges (WO-123 to WO-068, WO-120, WO-100 and
  WO-042; WO-055 to WO-042; WO-118 to WO-123): 109 nodes, 206 edges, no
  dangling edge, no cycle, and the set of filed orders with an empty
  blocking set is unchanged.
- The formatter check, the release surface check, the shell suites, the
  publication checks (the software-engineer edition's source lock refreshed
  for the product 07 sentence), the package tests, the harness checks and
  evidence, the plan-refutation fixtures, the corpus, artifact-identity,
  verification and feedback evidence checks and the mutation self-test
  passed. The plan-refutation suite's repository gate refused with
  `planning subject has uncommitted changes` before the commit; after it,
  the gate refuses this heading until a receipt matches the committed
  subject, by design, which the next direct-session receipt with the
  accepted dispositions supplies.
- The local-terms screen ran over every changed committed surface with the
  operator's list present and exit 0; `git diff --check` is clean.

The second repair revision's checks ran on the working tree before its
commit, after receipt 003 was committed at `1ae24bd` (subject hash
`sha256:818a769d492863f933e4c508bab72552b5ec62fb0c02173c35f01e4d8ae895b9`):

- `npm run work-orders -- index` regenerated and `index --check` passed;
  the workspace subject built with 76 orders and 394 acceptance criteria and
  no parse failure; WO-121 criterion 1 differs from the judged subject, the
  two earlier held criteria still differ from receipt 002's subject, and a
  dry run of the admission rules admits a receipt carrying the three dated
  dispositions and refuses one that omits the new hold's disposition.
- The graph gained one `hard` edge (WO-121 to WO-044): 109 nodes, 207
  edges, no dangling edge, no cycle.
- The full `npm test` chain passed every step before the plan gate,
  including the publication checks and the plan-refutation fixtures, and
  the steps after the gate (the corpus, artifact-identity, verification and
  feedback evidence checks and the mutation self-test) passed when run
  separately; the gate refused with `planning subject has uncommitted
changes` before the commit and, after it, refuses this heading until a
  receipt matches the committed subject, by design.
- The local-terms screen ran over every changed committed surface with the
  operator's list present and exit 0; `git diff --check` is clean.

The closing checks ran after receipt 004 was committed at `74a2064`, with
the subject unchanged at
`sha256:818a769d492863f933e4c508bab72552b5ec62fb0c02173c35f01e4d8ae895b9`:

- The receipt chain validates with four receipts; receipt 004 matches the
  committed and workspace subject, carries the three dated dispositions,
  and holds nothing; `npm run plan -- check` admits the heading.
- The full `npm test` chain passed end to end, including the plan gate and
  the mutation self-test.
- The formatter check, the local-terms screen (list present, exit 0) and
  `git diff --check` passed over the closing records; the workspace subject
  hash equals the committed and judged hash, so the records changed no
  refutation input.
