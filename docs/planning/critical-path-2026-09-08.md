...
references, so a finding could widen a writer's scope. Both criteria are
repaired in this revision, WO-118 names WO-123 as a hard input, WO-100
names the `intent` class as WO-123's, and the accepted dispositions are in
[critical-path-2026-09-08-dispositions.json](critical-path-2026-09-08-dispositions.json)
for the next receipt, which the operator requests the same way with those
dispositions; until it is filed, the plan gate refuses this heading by
design. No hold is overridden.
...
| Refutation gate cost                                        | The subject holds seventy-six orders; neither transport had budget for the first receipt, which is a direct-session review with session-attested independence. | The command, the direct-session form and the override route are recorded above; WO-125 makes the Codex transport selectable at `max`; the first hold was answered by two changed criteria. | A third consecutive hold stops the pass by rule.                                             |
