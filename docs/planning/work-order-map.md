...
activation. WO-020 now has local `v0.6.0` release evidence. WO-030 passed
independent verification and final review at this revision and awaits operator
merge; the index derives lifecycle facts anew instead of relying on this dated
planning sentence.
...
- The concurrent-workflow control slice is implemented in WO-030's source and
  ...
  operator merge; the lane-plan projection, declared per-order workflows,
...
| [WO-029](../work-orders/WO-029-pinned-artifact-identity.md)    | composition/runtime evidence — pin equipped artifact identity and refuse on drift                              | active v0.9.0 from v0.8.0; independently verify the pin/compare receipt, unchanged semantic preimage, logged cutover, and bounded WO-009 handoff                                                              | compiler/skeleton receipt implementer, then independent verifier                | pure compiler plus deterministic runtime/audit fixtures                                                 | compiler artifact identity; skeleton equip/recompile/refusal/audit path; product, map, and lineage docs                                                                                |
