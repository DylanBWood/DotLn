...
| [WO-064](../work-orders/WO-064-target-publish.md)                     | delivery adapter — push and pull request on a target under an explicit remote grant, body generated from artifacts                                                                                                                                                                             | WO-052, WO-063 and WO-042 merged; assign version at activation (minor); operator runs the smoke against a scratch remote; boy-scout nomination (REVIEW-002 pass, 2026-09-22, host-reviewed): build the work-order path pattern once per call in `scripts/lib/plan-subject.mjs` lines 309–314 so `plan check` runs under 2 s with the `plan-refutation` suite green; a declined item stays a register row                                                                                                   | control-plane implementer, then independent verifier                                                      | the CLI stub; a scratch remote                                                                                                                                        | `scripts/worktree.mjs`, `scripts/github-body.mjs`, product 07                                                                                                                                                    |
...
| [WO-100](../work-orders/WO-100-preauthorized-portfolio.md)            | runtime — the preauthorized portfolio and pure work derivation from the Gardener's candidates                                                                                                                                                                                                  | WO-068, WO-052, WO-054 and WO-042 merged; assign version at activation (minor); live feedback audit operator-run; boy-scout nomination (REVIEW-002 pass, 2026-09-22, host-reviewed): emit the shared refusals paragraph once in the floor and refer to it from each generated skill, only if a skill read without the floor stays complete in every supported harness; the bundle regeneration and edition re-mint are already criterion 5; a declined item stays a register row                                                                                                           | skeleton implementer, then independent verifier                                                           | doubles over the scratch target                                                                                                                                       | the configuration schema, `packages/skeleton/src/portfolio.ts` (new), the resident's activation path, products 03/06/07                                                                                          |
