...
**Phase two (2026-09-06 planning pass):** the 2026-09-05 horizon is complete
and every order in it ran serially. The new horizon turns toward the visible
loop the vision demands and the two fronts the operator named: a read-only
showrunner board inside core (WO-032) and a relocatable control plane that
exports a launchpad, drives registered target repositories, and gives lanes a
sync step (WO-033) form the first real paired wave; the cross-repository
workstream pilot through the operator's starter fork and `DotLn-Angular`
(WO-034) and the documentation structure reset (WO-035) form the second; the
evidence runner (WO-036) floats into any free lane, ideally between the
waves. Serial fallback is WO-032 → WO-033 → WO-035 → WO-034 with WO-036 in a
quiet window. The [phase-two plan](phase-two-plan-2026-09-06.md) holds the
sweep findings, the six-month projection, the vision-alignment review, the
write-surface table, the concurrency procedure, and the NoOp register. The
adjacent track is unchanged; WO-014 floats and gains weight once target-
repository sessions report approval friction. Version assignment follows the
opt-out default at activation: the first wave-1 order to merge takes the next
minor above `v0.13.1`, and the second retimes at its sync step. The operator
clarified that external organizations will fork the starter, the first within
about a week; WO-033 is therefore the order that timeline binds. The license
posture was decided inside the pass (`docs/LEGAL.md` §Decision) with the
license files landing in the planning pull request; WO-038, a small order
for the package metadata, publication guard, and contribution rule, leads the
sequence and runs in the first free lane before the first external fork. If
capacity is short, run WO-038 then WO-033 alone.
...
- WO-032 — Showrunner board
- WO-033 — Relocatable control plane
...
activation. The list is the serial reading order; the wave pairing (WO-032 ∥
WO-033, then WO-035 ∥ WO-034, WO-036 floating, WO-037 as the wave-3
candidate once WO-032 has merged) is in the phase-two plan. The
...
- **Sibling workflow pilot — filed 2026-09-06.** The 2026-09-06 pass filed
  the launchpad export and target-repository mechanics as WO-033 and the
  synthetic-plus-real pilot as WO-034; the route is recorded in the
  [phase-two plan](phase-two-plan-2026-09-06.md#the-enterprise-workflow-decision).
...
- The concurrent-workflow control slice shipped in WO-030 (`v0.7.0`); the
  first measured paired wave is phase two's wave 1 (WO-032 ∥ WO-033), and the
...
carry no direct-filing provenance.
...
| [WO-032](../work-orders/WO-032-showrunner-board.md)                  | projections/console — read-only showrunner board over control state, worktrees, releases, evidence, blueprint status                                                                     | wave 1 lane A; assign version at activation (minor; the second wave-1 merger retimes); activate from clean published main; no `scripts/` edits                                                             | projection/renderer implementer, then independent verifier                               | fixture inputs recorded from the documented commands; the real repository render                        | new `packages/console/`, root `console` script, product 04/13/06, README, capability table row, evidence WO-032                                                                        |
| [WO-033](../work-orders/WO-033-relocatable-control-plane.md)         | control plane — configuration root, target repositories with authority profiles and classes, launchpad export and update, lane sync, Beacon portability                                  | wave 1 lane B; assign version at activation (minor); confirm the two sibling repositories are still empty and operator-owned; `gh` stub for target publish fixtures                                        | control-plane implementer, then independent verifier                                     | real-Git launchpad-and-target fixtures; offline `npm ci` in the export fixture; no network              | `scripts/**`, the Beacon `.mjs` module home, `package.json` test order (boy-scout), product 03/07/10/12, playbook, ADR-0006 amendment, LEGAL note                                      |
| [WO-034](../work-orders/WO-034-cross-repository-workstream-pilot.md) | workstream application — synthetic six-demonstration pilot plus the operator-witnessed real run into `DotLn-Angular`                                                                     | WO-033 merged; WO-032 recommended; operator available to fork the starter, register the target, and witness the run; WO-038 landed before the first external fork                                          | pilot implementer, launchpad-dispatched sessions, operator witness, independent verifier | real-Git three-repository fixture; the operator's GitHub repositories for the real run                  | `docs/workstreams/` convention, `scripts/work-orders.mjs` grouping, `packages/console` section, product 12/04/13/06, evidence WO-034                                                   |
