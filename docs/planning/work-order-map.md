...
WO-039 close (previous revisions 2026-09-06, the phase-two pass after the
...
**Critical path (2026-09-08):** the operator's dispatch appended an external source-level audit and asked for a dependency-correct path from the current code to the first external source-changing worker and then to an independently verified source-to-deliverable vertical. The pass verified every material audit claim against `main` at `33e2c25` ([source verification](source-verification-2026-09-08.md)) and filed the [critical-path plan](critical-path-2026-09-08.md) with its [machine-readable graph](critical-path-2026-09-08.json). The horizon now leads with the authority floor ([WO-042](../work-orders/WO-042-authority-provenance.md): a linked support may only narrow a compiled build's authority, widening needs a provenance-bearing grant, and the tooltip projects the effective envelope), beside typed dependency truth ([WO-043](../work-orders/WO-043-typed-dependency-truth.md)) and the evidence runner (WO-036), which stays active. The rest of the path is recorded as unfiled candidates with entry criteria, in order: the writing-worker harness truth record, the target-worktree bundle emit, the single-repository source-changing worker (the first external source change and the first user-value proof), blinded verification over a real repository, the browser evidence adapter, SourceBundle to StoryContract, the target publish slice, and the Angular source-to-verified-PR vertical with its post-PR loop; runtime boundary codecs float beside them and are mandatory before the vertical. WO-033, WO-040, WO-034, WO-035 and WO-037 keep their numbers, scope and obligations but are deferred behind the first external source change and the mandatory replan that follows it; WO-033 is partially superseded in its target-worktree emit and target publish halves, which the path carries as slices. The first mandatory replan checkpoint is the writing-worker harness truth record. The pass did not run the refuter (it dispatches a model transport the sandboxed session cannot launch); the ledger section names the operator-run command and the possible hold on the deliberately unfiled loop order. The 2026-09-06 wave pairing below is superseded in sequence only; its measurements, procedure and receipts stand.
...
activation. The list is the serial reading order with the 2026-09-08
immediate gate first (WO-042, WO-043 and WO-036 may run in any free lanes)
and the deferred orders after it in their earlier relative order; the
critical-path candidates that sit between them are unfiled and listed in the
[critical-path plan](critical-path-2026-09-08.md), which also names the
replan checkpoint that precedes any deferred activation. The completed
2026-09-05 horizon (WO-020, WO-030, WO-021, WO-029, WO-009, WO-031, WO-022,
WO-010, WO-011) and the closed part of the 2026-09-06 horizon (WO-038,
WO-039, WO-041, WO-032) keep their closed evidence in the generated index;
their rationale is retained below for the record.
...
- **Critical-path candidates — recorded 2026-09-08 by the critical-path
  planning pass, unfiled, each with entry criteria in the
  [plan's activation briefs](critical-path-2026-09-08.md#activation-briefs).**
  In path order: runtime boundary codecs and the executable program split
  (floats; mandatory before the vertical); writing-worker harness truth in a
  foreign worktree (the first mandatory replan checkpoint follows its
  record); the target-worktree harness bundle (carved from WO-033 phase 2);
  the single-repository source-changing worker (the first external source
  change; carries the phase-two plan's recorded reactor-split condition);
  blinded verification and repair over a real repository (WO-010's loop with
  a live verifier); the browser evidence adapter (visual and network claims;
  needs an ADR-0002 dependency note); SourceBundle to StoryContract from a
  GitHub Issue; the target publish slice (carved from WO-033 phase 2); and
  the Angular source-to-verified-PR vertical with the post-PR loop, measured
  against the operator's v1 parity checklist. No number until each is filed;
  the plan is the authority for their order and entry criteria.
...
| [WO-033](../work-orders/WO-033-compiled-starter-export.md)           | control plane — configuration root, target repositories as builds (authority profile, class, profile, worktree-local harness emit), a launchpad export carrying the Contributor build and a pinned runtime build, kit/instance/overlay manifest, lane sync, Beacon portability                 | deferred 2026-09-08 behind the first external source change and its replan; its target-worktree emit and target publish halves are carried as critical-path slices; otherwise as before: assign version at activation (minor); real-Git fixtures; the build-first item now rides WO-036                                         | control-plane implementer, then independent verifier; the second wave merger records the wave receipt     | offline `npm ci` for the export fixture; `gh` stub; the local harness for the export's recorded smoke             |
| [WO-034](../work-orders/WO-034-cross-repository-workstream-pilot.md) | workstream application — synthetic six-demonstration pilot plus the operator-witnessed real run from a fork running the compiled build into `DotLn-Angular`, whose first change is a UIFA v1 shell over `uifa-board-v1`                                                                        | deferred 2026-09-08 behind the first external source change and its replan (its Angular-first-change premise is a recorded contradiction with the vertical, decided there); otherwise as before: after WO-033 and WO-032; assign version at activation (minor); the operator forks, registers the target, and witnesses the run | implementer for fixtures; launchpad-dispatched executor, verifier, and reviewer sessions for the real run | the operator's fork and target repositories; hook logs from the fork's sessions for the fired-unit counts         |
| [WO-035](../work-orders/WO-035-documentation-structure-reset.md)     | documentation structure — ledger order and index, spec/receipt boundary, generated release history, shorter cold start                                                                                                                                                                         | deferred 2026-09-08 behind the first external source change and its replan; WO-033 merged (roots); must not edit `work-orders.mjs` or `packages/console`; assign version at activation (patch)                                                                                                                                  | documentation/tooling implementer, then independent verifier                                              | repository-wide checks; no network                                                                                | ledger, `docs/lineage/`, product 02/03/06/10/14, capability table, execution guide, `scripts/lineage.mjs`, `scripts/docs-check.mjs`, publication index and locks                                                                    |
