...
| [WO-142](../work-orders/WO-142-outstanding-cleanup.md) | maintenance — outstanding cleanup: the register's refill, unowned review nominations, stale live-document claims and local residue, as 50 re-observed rows | first queued entry, lane pair with WO-084, by the operator's 2026-09-19 budgeted pass; assign version at activation (minor); row B3 may be returned and judged alone at the operator's word; the operator reads the prune listing before `--apply` | broad-surface repairer, then independent verifier judging the rows file | existing fixture suites; one live feedback edition; no new gate or hook | `scripts/`, `packages/*` except `reactor.ts`, the live documents, the regenerated bundle, indexes and editions; fenced off the ledger and `docs/lineage/README.md` |
