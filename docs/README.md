...
- 2026-09-08: WO-039 fourth repair regenerates the project hook runtime pins.
  ...
  its own reservation writes a new nonce-named file that names the one it
  supersedes, so a stale reclaimer cannot remove refreshed facts, and the
  operator release judges, retires and journals one observed reservation. The
  live-holder smoke's required denied effect is the writer guard's refusal.
  No skill, permission matcher or user-scope setting changes.

- 2026-09-07: WO-039 third repair regenerates the project hook runtime pins.
  The writer reservation becomes a directory instance whose recovery steps act
  only on the observed instance, so two sessions reclaiming the same dead
  holder admit exactly one writer; a pre-repair single-file reservation is
  migrated or reclaimed, never created. A refused Stop is reported once and
  its harness re-entry ends the turn with the obligation recorded; output
  refusals name twelve paths and the remaining count. No skill, permission
  matcher or user-scope setting changes.

- 2026-09-07: WO-039 second repair regenerates the project hook runtime pins.
  The writer reservation now records its owning harness process, reclaims a
  dead owner's reservation with a logged record, names a live holder in the
  refusal, and gains the operator commands `harness writer --show` (metadata)
  and `harness writer --release [--force]`. No skill, permission matcher or
  user-scope setting changes.

- 2026-09-07: WO-039 repair regenerates the project hook runtime pins and the
  executor/verifier/reviewer output-reading instructions. Verified ranges and
  bounded byte deliveries can complete large-output receipts; refusals identify
  missing paths. Scratch read auditing records attempted paths and counts, with
  native Read observation independent of scope enforcement. The work-order diff
  remains the output obligation; no user-scope setting or permission grant changes.

- 2026-09-07: WO-039 makes project permissions, hooks, role skills in `.claude/`
  and `.agents/`, and the marked `CLAUDE.md` residue compiler output. The floor
  keeps the locked source boundary and replaces the mandatory whole-guide read
  with role skill dispatch. `harness check` owns generated bytes; the config
  authority and unchanged personal posture are recorded in ADR-0005's amendment.
  No user-scope settings changed.
