...
await test("test-control-beacons", async (t) => {
  let owned,
    ...
    publicDir;
  await t.test(
    "three-worktree metadata constellation (opaque fixture addresses shortened here):",
    () => {
      owned = process.argv[2];
      ...
      root = join(owned, "constellation project");
      ...
      resume = (cwd, args) => ok(call(cwd, "resume", args));

      status = (cwd) => JSON.parse(resume(cwd, ["status", "--json"]));
      ...
      token = issueBeaconSession(root, "WO-097");

      resume(root, [
        "activate",
        "WO-097",
        "docs/work-orders/WO-097-fixture.md",
      ]);
      resume(left, [
        "activate",
        "WO-098",
        "docs/work-orders/WO-098-fixture.md",
      ]);
      ...
      observations = sweepControlBeacons(worktrees);
      ...
      before = [root, left, right].map(snapshotBeacons);

      logsBefore = [root, left, right].map((checkout) =>
        readFileSync(
          join(
            checkout,
            `docs/control/orders/${status(checkout).workOrder}.jsonl`,
          ),
        ),
      );

      output = ok(call(root, "worktree", ["constellation"]));
  ...
  await t.test(
    "unreadable garbage content still decodes; missing expected worktree beacon is absent",
    () => {
      console.log(output.replaceAll(/[a-f0-9]{64}/g, "fixture-address").trim());

      file = join(controlBeaconDirectory(left), controlBeaconAddress("WO-098"));
  ...
  await t.test(
    "verifier contains only host records; two independent 256-bit names; restricted name appears only in the authorized active next briefing",
    () => {
      privateNext = resume(root, ["next", "--beacon-session", token]);
      ...
      wrongScope = call(left, "resume", ["next", "--beacon-session", token]);

      assert.equal(wrongScope.status, 1);
      assert.ok(!wrongScope.stderr.includes(name));
      otherOrderToken = issueBeaconSession(root, "WO-098");

      activeWrongScope = call(root, "resume", [
        "next",
        "--beacon-session",
        otherOrderToken,
      ]);
  ...
  await t.test(
    "anchored Beacon disposal; protected intake and lookalike/nested paths still refuse",
    () => {
      for (const path of [
        ".control-beacons",
  ...
  await t.test(
    "filesystem projection failure records exactly one transition and warns without leaking restricted paths",
    () => {
      // Projection failure is loud but cannot reverse an already-appended transition.
      publicDir = controlBeaconDirectory(root);
