...
export async function buildAllCorpus(seed) {
  const kernelUrl = pathToFileURL(
    `${REPO_ROOT}/packages/kernel/dist/src/index.js`,
  ).href;
...
export async function runGenerator(lane, argv) {
  ...
  if (lane !== "program" && lane !== "ids")
    throw new Error(`unknown generator lane ${lane}`);
  ...
  const laneSummary =
    lane === "program" ? corpus.program.summary : corpus.ids.summary;
  process.stdout.write(
    `${JSON.stringify({ workOrderId: "WO-101", lane, mode, seed, files: files.length, summary: laneSummary })}\n`,
  );
}
