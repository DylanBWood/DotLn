...
const recoveryInput = input !== null && typeof input === "object" && !Array.isArray(input) && (input.prompt === undefined || typeof input.prompt === "string") && (typeof input.session_id === "string" || ("UserPromptSubmit" === "UserPromptSubmit" && /^(analysis|operator override):(?:\s|$)/i.test((input.prompt ?? "").trim())));
...
})({ ...input, session_id: typeof input.session_id === "string" ? input.session_id : undefined }, "UserPromptSubmit") : null;
if (control) { process.stdout.write(JSON.stringify(control)); } else {
const { feedbackBoundary } = await import("../../.runtime/harness/ebbad1641e22c96d/packages/skeleton/dist/src/feedback-boundary.js");
const { runHarnessHook } = await import("../../.runtime/harness/ebbad1641e22c96d/packages/skeleton/dist/src/harness-host.js");
await runHarnessHook({
  "compilerPackageVersion": "0.11.1",
  "runtime": {
    "skeletonVersion": "0.18.2",
    ...
    "files": [
      ...
      },
      {
        ...
        "hash": "fnv1a64:cd1922624e641ad1"
      }
    ],
    "snapshot": ".runtime/harness/ebbad1641e22c96d"
  },
  "event": "UserPromptSubmit",
  "tools": {
    "Read": "read",
...
try {
  ...
  if (!fs.existsSync(path) || fs.lstatSync(path).isFile())
    fs.appendFileSync(path, JSON.stringify({ recordedAt: new Date().toISOString(), event: "UserPromptSubmit", advisory: response.systemMessage, delegated: true }) + "\n", { mode: 0o600 });
} catch {}
