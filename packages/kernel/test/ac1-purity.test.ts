...
const predicates: PredicateRegistry = {
  ...
  "presence.away-after": {
    1: ({ env: environment }, params) =>
      environment.now >= Number(params["threshold"]),
  },
};
...
test("AC1 evidence: no ambient Date.now or Math.random read on either branch (poisoned globals)", () => {
  ...
  Date.now = (): number => {
    throw new Error("kernel read ambient Date.now");
  };
  Math.random = (): number => {
    throw new Error("kernel read ambient Math.random");
  };
  try {
    for (const presence of ["returned", "away"] as const) {
      for (const ids of [
        ...
        const decision = guardQueuedPulse(
          { presence },
          event(`pulse-${presence}-${ids.length}`, "CadencePulse", 60),
          env(60, 8),
          awayRef,
          ids,
        );
        assert.deepEqual(decision.trace.branchPath, [
          "queued-pulse",
          presence === "away" ? "active" : "noop",
        ]);
        assert.deepEqual(
          decision.cancelledScheduleIds,
          presence === "away" ? [] : ids,
        );
      }
    }
...
test("AC1 evidence: identical decisions across a simulated one-hour ambient clock gap", () => {
  ...
  try {
    for (const presence of ["returned", "away"] as const) {
      ...
      const first = guardQueuedPulse(state, pulse, env(77, 13), awayRef, [
        "s-2",
        "s-9",
        "s-4",
      ]);
...
test("AC1 evidence: cancelledScheduleIds preserves caller order for multi-element schedule lists", () => {
  ...
  const decision = guardQueuedPulse(
    { presence: "returned" },
    event("pulse-order", "CadencePulse", 10),
    env(10, 3),
    awayRef,
    ids,
  );
...
test("AC1 evidence: full-decision property across varied state, presence, now and rngState", () => {
  ...
  for (let seed = 0; seed < 120; seed++) {
    ...
    const state = {
      presence,
      seed,
      tags: [`t-${seed}`, `t-${seed + 1}`],
      nested: { depth: seed % 5 },
    };
    const pulse = event(`pulse-${seed}`, "CadencePulse", 1000 + seed * 17);
    const environment = env(1000 + seed * 17, seed * 31 + 5);
    const ids = [`s-${seed}-b`, `s-${seed}-a`, `s-${seed}-c`].slice(
      0,
      seed % 4,
    );
    const ref: PredicateRef =
      seed % 3 === 0
        ? { registryId: "presence.away", version: 1, params: { seed } }
        : awayRef;
    ...
    const expected: PresenceDecision<typeof state> = {
        ...
        : [
            {
              ...
              reevaluation: { kind: "Once", at: pulse.occurredAt },
              usefulWhen: ref,
            },
          ],
      schedules: [],
      ...
      trace: {
        ...
        cadenceEvaluations: away ? [] : ids.map((id) => `cancel:${id}`),
      },
    };
...
test("AC1 evidence: env.now reaches the predicate and flips the decision branch", () => {
  const ref: PredicateRef = {
    registryId: "presence.away-after",
    version: 1,
    params: { threshold: 500 },
  };
  const state = { presence: "irrelevant" };
  const pulse = event("pulse-env", "CadencePulse", 480);
  const before = guardQueuedPulse(state, pulse, env(480, 1), ref, [
    "s-2",
    "s-9",
    "s-4",
  ]);
  const atThreshold = guardQueuedPulse(state, pulse, env(500, 1), ref, [
    "s-2",
    "s-9",
    "s-4",
  ]);
