...
test("AC4 evidence: Invoke refuses a foreign commandId — residual deep-equals pre-step program, intents empty, no re-dispatch on any unrelated event", () => {
  ...
  assert.deepEqual(dispatch.residual, {
    kind: "Invoke",
    commandId: "cmd_1",
    command: {
      kind: "Act",
  ...
  const wrongId = stepProgram(
    program,
    { flag: true },
    env(),
    event("r-wrong", "CommandResult", 5, {
      commandId: "cmd_other",
      result: "ok",
    }),
  );
  ...
  for (const unrelatedEvent of unrelated) {
    ...
    assert.deepEqual(
      step.intents,
      [],
      `re-dispatched on ${unrelatedEvent.eventId}`,
    );
    assert.deepEqual(
      step.residual,
      program,
      `advanced on ${unrelatedEvent.eventId}`,
    );
...
test("AC4 evidence: Invoke ignores event type — a non-CommandResult event carrying a matching payload must not advance", () => {
  const program = invokeProgram();
  const masquerade = stepProgram(
    program,
    { flag: true },
    env(),
    event("masq", "Approved", 5, { commandId: "cmd_1", result: "ok" }),
  );
  ...
  const advanced = stepProgram(
    program,
    { flag: true },
    env(),
    event("real", "CommandResult", 5, { commandId: "cmd_1", result: "ok" }),
  );
...
test("AC4 evidence: Await ignores wrong correlationId and wrong type — residual stays the Await and the waits channel reports it", () => {
  const awaitNode = Program.Await(
    { type: "Approved", correlationId: "corr-1" },
    Cadence.After(1000),
    Program.Done(),
  );
  const wrongCorrelation = stepProgram(awaitNode, {}, env(), {
    ...event("a1", "Approved", 5),
  ...
  const wrongType = stepProgram(awaitNode, {}, env(), {
    ...event("a2", "Rejected", 5),
  ...
  const matched = stepProgram(awaitNode, {}, env(), {
    ...event("a3", "Approved", 5),
    correlationId: "corr-1",
  });
...
test("AC4 evidence: decideProgram returns the full literal decision on the event path, including waits-derived cadenceEvaluations", () => {
  ...
  const pending = decideProgram(awaitNode, { flag: false }, env(), {
    ...event("a1", "Approved", 5),
    correlationId: "corr-2",
  });
  assert.deepEqual(pending, {
    ...
    continuation: {
      kind: "Await",
      pattern: { type: "Approved", correlationId: "corr-1" },
      timeout: { kind: "After", delayMs: 1000 },
      next: { kind: "Done" },
    },
    schedules: [],
    trace: {
      reactorId: "program",
      reactorVersion: "1",
      branchPath: ["Await", "Await"],
      envInputs: ["event"],
      cadenceEvaluations: ["await:Approved"],
    },
    emitted: [],
    waits: [
      {
  ...
  const advanced = decideProgram(
    invokeProgram(),
    { flag: true },
    env(),
    event("r1", "CommandResult", 5, { commandId: "cmd_1", result: "ok" }),
  );
...
test("AC4 evidence: three-step predicate-ref program round-trips losslessly at every step with a structural pre-serialization closure check", () => {
  ...
  assert.throws(
    () => assertNoFunctions({ bad: () => 0 }, "probe"),
    /closure found at probe\.bad/,
  );
  assert.throws(
    () => assertNoFunctions({ nested: { deep: [Math.max] } }, "probe"),
    /closure found at probe\.nested\.deep\.0/,
  );

  const finished = draft("Finished", 0);
  const awaitNode = Program.Await(
    { type: "Approved", correlationId: "corr-1" },
    Cadence.Until(Cadence.After(1000), flagRef),
    Program.Guard(
      flagRef,
  ...
  step = stepProgram(
    ...
    { flag: true },
    env(),
    event("r-foreign", "CommandResult", 4, {
      commandId: "cmd_other",
      result: "ok",
    }),
  );
  ...
  step = stepProgram(
    ...
    { flag: true },
    env(),
    event("r-match", "CommandResult", 5, { commandId: "cmd_1", result: "ok" }),
  );
  ...
  assert.deepEqual(step.residual, {
    kind: "Sequence",
    programs: [
      Program.Guard(
        flagRef,
...
test("AC4 evidence (B1 regression): prototype-key results throw No Invoke continuation and never resolve native functions", () => {
  ...
  assert.throws(
    () =>
      stepProgram(
        ...
        { flag: true },
        env(),
        event("p-unknown", "CommandResult", 5, {
          commandId: "cmd_1",
          result: "nope",
        }),
      ),
  ...
  const advanced = stepProgram(
    ...
    { flag: true },
    env(),
    event("p-ok", "CommandResult", 5, { commandId: "cmd_1", result: "ok" }),
  );
  assert.deepEqual(advanced.residual, { kind: "Done" });
