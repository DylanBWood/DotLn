...
import type { Event, EventEnvelope, JsonValue, KernelEnv, PredicateRegistry } from "../src/index.js";
...
const registry: PredicateRegistry = {
  ...
  "event.is-open": { 1: ({ event: triggering }) => triggering?.type === "OpenGate" },
};
const env = (now: number, rngState: number, predicates: PredicateRegistry = registry): KernelEnv => ({ now, rngState, predicates });
...
test("AC3 evidence: Backoff jitter yields exact dueAt and rngState for pinned rngStates with a non-binding clamp", () => {
  ...
  assert.deepEqual(evaluateCadence(backoff, {}, env(100, 42)), { dueAt: 702, rngState: 1083814273, trace: "Backoff:3:602" });
  assert.deepEqual(evaluateCadence(backoff, {}, env(100, 7)), { dueAt: 691, rngState: 1025555898, trace: "Backoff:3:591" });
  assert.deepEqual(evaluateCadence(backoff, {}, env(0, 1)), { dueAt: 589, rngState: 1015568748, trace: "Backoff:3:589" });
});
...
test("AC3 evidence: Backoff rngState chains — successive draws differ and are reproducible", () => {
  ...
  assert.deepEqual(first, { dueAt: 702, rngState: 1083814273, trace: "Backoff:3:602" });
  assert.deepEqual(second, { dueAt: 571, rngState: 378494188, trace: "Backoff:3:471" });
  ...
  assert.deepEqual(evaluateCadence(backoff, {}, env(100, first.rngState)), second);
});
...
test("AC3 evidence: Backoff floor clamps a negative jittered delay to zero, never due before now", () => {
  ...
  assert.deepEqual(evaluateCadence(Cadence.Backoff(100, 2, 100000, 3, 2), {}, env(100, 7)), { dueAt: 100, rngState: 1025555898, trace: "Backoff:3:0" });
});
...
test("AC3 evidence: Backoff draws jitter from explicit rngState only — ambient Math.random and Date.now poisoned", () => {
  ...
  try {
    Math.random = () => { throw new Error("ambient Math.random consulted"); };
    Date.now = () => { throw new Error("ambient Date.now consulted"); };
    assert.deepEqual(evaluateCadence(Cadence.Backoff(100, 2, 100000, 3, 0.5), {}, env(100, 42)), { dueAt: 702, rngState: 1083814273, trace: "Backoff:3:602" });
  } finally {
...
test("AC3 evidence: Every before startAt yields exactly startAt", () => {
  const rows: readonly (readonly [intervalMs: number, startAt: number, now: number])[] = [[20, 50, 10], [20, 50, 49], [20, 50, 0], [1000, 999999, 5]];
  for (const [intervalMs, startAt, now] of rows) {
    const result = evaluateCadence(Cadence.Every(intervalMs, startAt), {}, env(now, 7));
    assert.equal(result.dueAt, startAt, `Every(${intervalMs}, ${startAt}) at now=${now}`);
...
test("AC3 evidence: Every has no fixed points — grid of now/startAt/interval yields the exact next tick strictly after now", () => {
  const rows: readonly (readonly [intervalMs: number, startAt: number | undefined, now: number, expected: number])[] = [
  ...
  for (const [intervalMs, startAt, now, expected] of rows) {
    const result = evaluateCadence(Cadence.Every(intervalMs, startAt), {}, env(now, 7));
    assert.equal(result.dueAt, expected, `Every(${intervalMs}, ${startAt}) at now=${now}`);
    assert.ok(result.dueAt !== null && result.dueAt > now, `Every(${intervalMs}, ${startAt}) at now=${now} must be strictly after now, got ${result.dueAt}`);
  }
});
...
test("AC3 evidence: Until open path recurses into the inner cadence verbatim", () => {
  ...
  assert.deepEqual(evaluateCadence(Cadence.Until(Cadence.Every(20, 50), flagRef), { flag: false }, env(110, 7)), { dueAt: 130, rngState: 7, trace: "Every:20" });
  // Inner Backoff: the pinned rngState 42 literals must come through untouched, rng threading included.
  assert.deepEqual(evaluateCadence(Cadence.Until(Cadence.Backoff(100, 2, 100000, 3, 0.5), flagRef), { flag: false }, env(100, 42)), { dueAt: 702, rngState: 1083814273, trace: "Backoff:3:602" });
  // Closed path for contrast: cancelled, no recursion.
  assert.deepEqual(evaluateCadence(Cadence.Until(Cadence.Every(20, 50), flagRef), { flag: true }, env(110, 7)), { dueAt: null, rngState: 7, trace: "Until:cancelled" });
});

test("AC3 evidence: predicate registry selects the ref's version", () => {
  const gateV1 = Cadence.Gate(Cadence.After(10), { registryId: "gate.version", version: 1 });
  const gateV2 = Cadence.Gate(Cadence.After(10), { registryId: "gate.version", version: 2 });
  assert.deepEqual(evaluateCadence(gateV1, {}, env(100, 7)), { dueAt: null, rngState: 7, trace: "Gate:closed" });
  assert.deepEqual(evaluateCadence(gateV2, {}, env(100, 7)), { dueAt: 110, rngState: 7, trace: "After:10" });
});

test("AC3 evidence: predicate registry passes ref params through to the predicate", () => {
  const gate = (params?: Readonly<Record<string, JsonValue>>): Cadence.Gate => Cadence.Gate(Cadence.After(10), params === undefined ? { registryId: "params.open", version: 1 } : { registryId: "params.open", version: 1, params });
  assert.equal(evaluateCadence(gate({ open: true }), {}, env(100, 7)).dueAt, 110);
  assert.equal(evaluateCadence(gate({ open: false }), {}, env(100, 7)).dueAt, null);
...
test("AC3 evidence: predicate registry threads the triggering event into the predicate context", () => {
  const gate = Cadence.Gate(Cadence.After(10), { registryId: "event.is-open", version: 1 });
  assert.equal(evaluateCadence(gate, {}, env(100, 7), event("e1", "OpenGate", 90)).dueAt, 110);
  assert.equal(evaluateCadence(gate, {}, env(100, 7), event("e2", "OtherEvent", 90)).dueAt, null);
...
test("AC3 evidence: unknown predicate or unknown version throws naming the id", () => {
  assert.throws(() => evaluateCadence(Cadence.Gate(Cadence.After(10), { registryId: "no.such", version: 1 }), {}, env(100, 7)), /no\.such@1/);
  assert.throws(() => evaluateCadence(Cadence.Gate(Cadence.After(10), { registryId: "gate.version", version: 99 }), {}, env(100, 7)), /gate\.version@99/);
});

test("AC3 evidence: Gate consults its predicate — false yields null dueAt, and the predicate is actually invoked", () => {
  assert.deepEqual(evaluateCadence(Cadence.Gate(Cadence.Once(150), flagRef), { flag: false }, env(100, 7)), { dueAt: null, rngState: 7, trace: "Gate:closed" });
  const calls: Array<{ readonly params: JsonValue; readonly eventType: string | null }> = [];
  const recording: PredicateRegistry = { "record.calls": { 1: ({ event: triggering }, params) => { calls.push({ params, eventType: triggering?.type ?? null }); return true; } } };
  const trigger: Event = event("pulse-1", "CadencePulse", 90);
  const result = evaluateCadence(Cadence.Gate(Cadence.Once(150), { registryId: "record.calls", version: 1, params: { tag: "g1" } }), {}, env(100, 7, recording), trigger);
  assert.deepEqual(result, { dueAt: 150, rngState: 7, trace: "Once:150" });
  assert.deepEqual(calls, [{ params: { tag: "g1" }, eventType: "CadencePulse" }]);
});
