...
This source prepares DotLn `v0.12.0`. Blinded verifier episodes now return claim-typed evidence and findings through the worker transports. A focused repair runs in a fresh episode, affected evidence becomes stale, and another verifier checks the changed subject. `dotln status` exposes the living acceptance matrix. The runnable demo and acceptance evidence use a synthetic repository and deterministic workers; live model verification and general source-writing workers have not been witnessed here. See the [verification runbook](packages/skeleton/README.md#independent-verification) and [WO-010 evidence](docs/evidence/WO-010/README.md). Existing inspection workers, Beacon senses and actor-usage reports remain available. Published source records are on the [GitHub Releases page](https://github.com/DylanBWood/DotLn/releases), or available locally through `npm run release -- notes <tag>`.
<!-- DOTLN-RELEASE-END -->
