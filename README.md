...
This source prepares DotLn `v0.11.0`. Beacon Sight, Fine Spectrum and Composition now compile as perception supports, require an explicit host mount and Observe grant, and expose only their declared metadata fields. Sparse v3 Beacons add a weak keyed provenance check; a deterministic mounted verifier demonstrates that implementer prose stays outside its inputs. The existing disposable inspection workers and actor-usage report remain available. Native model verification and general write-capable workers remain planned. See the [Senses runbook](packages/skeleton/README.md#senses) and [WO-022 evidence](docs/evidence/WO-022/README.md). Published source records are on the [GitHub Releases page](https://github.com/DylanBWood/DotLn/releases), or available locally through `npm run release -- notes <tag>`.
<!-- DOTLN-RELEASE-END -->
