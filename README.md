...
This source is DotLn `v0.4.1`. Its executable heart remains the deterministic walking skeleton and zero-runtime-dependency composition compiler introduced at the previous boundary; this patch consolidates lifecycle helpers, makes canonical control state machine-readable, protects ignored intake and local harness state across closeout, and derives cadence compatibility from type-exhaustive built-kernel data. Five linked Seiri supports still lower to a bounded WorkOrder, permission guard, evidence schema, absent-only cadence/statechart guard, and verifier episode; code DSL, function-table, and statechart-JSON views produce one semantic hash and one inspectable RPG tooltip. The executor and verifier remain deterministic fakes, and no model is in the loop yet. Read published records on the [GitHub Releases page](https://github.com/DylanBWood/DotLn/releases), or render one locally with `npm run release -- notes <tag>`.
<!-- DOTLN-RELEASE-END -->
